...
class BuildPjam < Struct.new( :build_async, :project, :build, :settings  )
    ...
    def _create_distribution_archive source
        ...
        cmd <<  "#{settings.perl5lib_as_shell_variable} perl Build.PL --quiet 1>/dev/null"
    ...
    def _install_pinto_distribution archive_name
        _execute_command("#{settings.perl5lib_as_shell_variable} pinto -r #{settings.pinto_repo_root} install -s #{project.id} -v --no-color -o 'v' -l #{project.local_path}/cpanlib  PINTO/#{archive_name}") 
    end
